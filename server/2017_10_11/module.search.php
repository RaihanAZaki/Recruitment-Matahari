...
if(isset($_SESSION["log_auth_id"]) && isset($_SESSION["log_auth_name"]) && isset($_SESSION["priv_id"]) && in_array($_SESSION["priv_id"],array("admin","hrd","pic")) ) {
	...
	function adm_search() {
		...
		if($chk_candidate_name=="1" && $candidate_name<>"") {
			$link .="&candidate_name=".$candidate_name;
		}
		if($chk_candidate_email=="1" && $candidate_email<>"") {
			$link .="&candidate_email=".$candidate_email;
		}
		...
		if($chk_candidate_edu_major=="1" && $candidate_edu_major<>"") {
			$link .="&candidate_edu_major=".$candidate_edu_major;
		}
		...
		if($chk_candidate_jobexp_lob=="1" && $candidate_jobexp_lob<>"") {
			$link .="&candidate_jobexp_lob=".$candidate_jobexp_lob;
		}
		if($chk_candidate_jobexp_position=="1" && $candidate_jobexp_position<>"") {
			$link .="&candidate_jobexp_position=".$candidate_jobexp_position;
		}
		if($chk_candidate_c_city=="1" && $candidate_c_city<>"") {
			$link .="&candidate_c_city=".$candidate_c_city;
		}
		if($chk_candidate_religion=="1" && $candidate_religion<>"") {
	...
	function adm_getSearchResult($start,$limit) {
		...
		if(mysql_query($maketemp)) {
			...
			$query_candidate_id_edu=querying("SELECT candidate_id FROM m_candidate_edu GROUP BY candidate_id");
			$candidate_id_edu = sqlGetData($query_candidate_id_edu);
			
			for($iedu=0;$iedu<count($candidate_id_edu);$iedu++)
			{
				querying("INSERT INTO edu_tmp(candidate_id, degree_name, edu_major, candidate_edu_id, candidate_edu_institution, candidate_edu_gpa, candidate_edu_end) 
					SELECT a.candidate_id, a.candidate_edu_degree, a.candidate_edu_major, a.candidate_edu_id, a.candidate_edu_institution, a.candidate_edu_gpa, a.candidate_edu_end
					FROM m_candidate_edu a
					WHERE candidate_id = '".$candidate_id_edu[$iedu]["candidate_id"]."'
					ORDER BY field( candidate_edu_degree, 'Doctoral - S3', 'Master - S2', 'Bachelor - S1', 'Diploma', 'Highschool - SMA', 'Junior Highschool - SMP', 'Elementary - SD' )
					LIMIT 1"); 
			}
			
			
			// querying("INSERT INTO edu_tmp(candidate_id, degree_id, edu_major, candidate_edu_id, candidate_edu_institution, candidate_edu_gpa) 
			// SELECT a.candidate_id, MAX(b.degree_id) as degree_id, a.candidate_edu_major, a.candidate_edu_id, a.candidate_edu_institution, a.candidate_edu_gpa 
			// FROM m_candidate_edu a LEFT JOIN m_degree b ON a.candidate_edu_degree=b.degree_name GROUP BY a.candidate_id");
		...
		if($candidate_edu_major<>"") {
			$query .=" AND d.edu_major LIKE '".$candidate_edu_major."'";
		}
		...
		if($candidate_jobexp_lob<>"") {
			$query .=" AND c.candidate_jobexp_lob LIKE '".$candidate_jobexp_lob."'";
		}
		if($candidate_jobexp_position<>"") {
			$query .=" AND c.candidate_jobexp_position = '".$candidate_jobexp_position."'";
		}
		if($candidate_c_city<>"") {
			$query .=" AND a.candidate_c_city LIKE '".$candidate_c_city."'";
		}
