...
function register_checkIDCard($idcard_number) {
	$query = querying("SELECT register_id FROM m_register WHERE candidate_idcard = ? ORDER BY register_id ASC LIMIT 1",array($idcard_number));
	$d_id_reg = sqlGetData($query);

	$query=querying("SELECT candidate_id FROM m_candidate WHERE candidate_idcard = ? ORDER BY candidate_id ASC LIMIT 1",array($idcard_number));
		...
		exit;
			...
			<b>Hormat kami,<br><br/>Tim Rekrutmen Matahari Putra Prima</b></p>";
				...
				if (function_sending_email($variablemail))
				{
					unset($_SESSION["session"]);
					unset($variablemail);
					header("location: "._PATHURL."/index.php?mod=resendactivation&mess=".coded("Please kindly check your email and find the activation link we had sent.<br><i>Silahkan periksa email Anda. Kami telah mengirimkan email aktifasi ke email Anda.</i>"));
					exit;	
				}
				else
				{
